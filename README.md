# Homework for MATH 373

This is the pretext development files for the homework in MATH 373. 
The main source file is `source/main.ptx`.  This includes the other homework assignments listed by number.

This is developed in github codespace by picking the codespaces option from the green button at the start. Once the codespaces is running (takes a long time to start up) you can click the pretext button at the bottom and build the project first then after it is built you can view the html by clicking on the pretext button and selecting the option to view full document and select the web print. The browser will attempt to pull up the file before the file is built so you need to refresh once you get a new window. You need to refresh after every modification to view the modifications. 

To compile a pdf, run `pretext build print`.

To save work go to the terminal and do the following
git status
git add .
git commit -m "describe upate"
git push
